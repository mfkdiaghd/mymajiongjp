...
function repair_point() {
    let round = $('input[name="順位点四捨五入有"]').prop('checked');
    let sum = 0;
    for (let i = 1; i < 4; i++) {
...
function repair_gang() {
    if (+ $('input[name="裏龙有"]:checked').val()
        && + $('input[name="カン龙有"]:checked').val())
    {
        $('input[name="カン裏有"]').prop('disabled', false);
    }
    else {
        $('input[name="カン裏有"]').prop('disabled', true).val([0]);
    }

    if (+ $('input[name="カン龙有"]:checked').val()) {
        $('input[name="カン龙後乗せ"]').prop('disabled', false);
    }
    else {
        $('input[name="カン龙後乗せ"]').prop('disabled', true)
                                        .prop('checked', false);
    }
}

function repair_damanguan() {
    if (+ $('input[name="役満の複合有"]:checked').val()) {
        $('input[name="ダブル役満有"]').prop('disabled', false);
    }
    else {
        $('input[name="ダブル役満有"]').prop('disabled', true).val([0]);
    }
}
...
$(function(){
    ...
    $('input[name="順位点四捨五入有"]').on('change', repair_point);
    $('input[name="赤牌"]').on('change', function(){
        let n = $(this).val();
        if (isNaN(n) || n < 0 || 4 < n) $(this).val(0);
    });
    $('input[name="裏龙有"]').on('change', repair_gang);
    $('input[name="カン龙有"]').on('change', repair_gang);
    $('input[name="役満の複合有"]').on('change', repair_damanguan);
